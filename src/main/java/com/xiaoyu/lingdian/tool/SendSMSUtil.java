...
public class SendSMSUtil {
	...
	public static boolean sendRegister(String mobileTel, String code) {
		String content = "";
		try {
			content = URLEncoder.encode("您在九管家微信平台的注册验证码："+code+",十分钟内有效.号外号外:即日起通过九管家官方微信平台下单寄快递,立可享受首单寄件9折优惠!", "UTF-8");
		} catch (UnsupportedEncodingException e) {
			System.out.println("注册发送短信验证信息编码失败！");
			e.printStackTrace();
		}
		String param = "loginname="+ConfigIni.getIniStrValue("SEND_SMS", "loginname", "271")+
				"&password="+ConfigIni.getIniStrValue("SEND_SMS", "password", "0KiEXDYag1")
				+"&mobile="+mobileTel+"&content="+content+"&extNo=";	
		String result = HttpUrlConnectionUtil.sendGet(
				ConfigIni.getIniStrValue("SEND_SMS", "path", "http://121.40.60.163:8081/message/sendMsg"), param);
